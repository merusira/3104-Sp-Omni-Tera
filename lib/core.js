...
module.exports = function SPCore(mod) {
	...
	// Projectiles
		...
		serverProjectiles = {}
	// Function to load the skills_client.json file
	function loadSkillsClient() {
		try {
			// Delete the cached version of the module
			delete require.cache[require.resolve('../config/skills_client.json')]
			// Load the file
			return require('../config/skills_client.json')
		} catch (error) {
			console.error(`[SkillPrediction] Error loading skills_client.json:`, error)
			return {}
		}
	}

	// Initialize skillsClient
	let skillsClient = {}
	
	mod.hook('S_LOGIN', 'raw', {order: 100, filter: {fake: null}}, event => {
		skillsCache.clear()
		
		// Reload the skills_client.json file
		skillsClient = loadSkillsClient()
	})
