...
// Cache config files for better performance
const skillsClient = require('../config/skills_client'),
	skills = require('../config/skills'),
	passivity = require('../config/passivity')

// Convert silence array to Set for faster lookups
const silence = new Set(require('../config/silence'))

// Constants
const TYPE_ACTION = 1,
	TYPE_REACTION = 2

// Cache end reasons in a lookup object
const END_REASONS = Object.freeze({
	stand: 5,
	lockonCast: 36
})

// Skill cache size limit to prevent memory leaks
const SKILL_CACHE_LIMIT = 1000

module.exports = function SPCore(mod) {
	// Lazy load dependencies
	let _player, _ping, _abnormality, _cooldowns
	
	// Define getters for lazy loading
	const player = () => _player || (_player = subMod(mod, './player'))
	const ping = () => _ping || (_ping = subMod(mod, './ping'))
	const abnormality = () => _abnormality || (_abnormality = subMod(mod, './abnormalities'))
	const cooldowns = () => _cooldowns || (_cooldowns = subMod(mod, './cooldowns'))
	...
	// Optimized sendActionStage function
	function sendActionStage(opts) {
		// Clear any existing server timeout
		mod.clearTimeout(serverTimeout)
		...
		if(info.dcType === 'movingcharge') {
			realSpeed = (realSpeed + opts.chargeSpeed) * info.timeRate
		}
		
		if(info.fixedSpeed) {
			animSpeed = 1
			// Check if we need to modify realSpeed based on dcType
			const skipRealSpeedReset = ['movingdefence', 'movingskill', 'shootingmovingskill'].includes(info.dcType)
			if(!skipRealSpeedReset) realSpeed = 1
		}

		// Debug logging if enabled
		if(mod.settings.debug.skills) {
			debug(`<* S_ACTION_STAGE ${skillId(opts.skill)} ${opts.stage} ${decimal(animSpeed, 3)}x ${decimal(realSpeed, 3)}x`)
		}
		...
		// Apply movement from previous stage
		movePlayer(opts.distance * opts.distanceMult, moveDir)
		...
		const packetData = {
			...
			moving: false,
			dest: info.setEndpointStage === opts.stage && opts.endpoints ? opts.endpoints[0].loc : undefined,
			target: 0,
			
			// Meta
			defendSuccess: false,
			overcharge: opts.overcharge || false
		}
		
		// Set defendSuccess if applicable
		if(opts.stage > 0 && clientAction && clientAction.skill === opts.skill) {
			packetData.defendSuccess = clientAction.defendSuccess
		}
		
		// Set overcharge if applicable
		if(clientAction && clientAction.overcharge) {
			packetData.overcharge = true
		}

		// Send action stage packet
		mod.send('S_ACTION_STAGE', 9, clientAction = packetData)

		// Update distance
		opts.distance = get(info, 'distance', opts.stage) || info.distance || 0
		stageEnd = null

		// Check if we need to skip timeout
		const noTimeout = serverAction &&
			Math.floor(serverAction.skill.id / 100) === Math.floor(opts.skill.id / 100) &&
			serverAction.stage >= opts.stage

		// Handle special distance types
		if(info.type === 'dynamicDistance') {
			opts.distance = myPosition.loc.dist2D(opts.dest)
		}
		else if(info.dcType === 'shorttel') {
			// Trigger teleport on last stage
			if(!info.length || !(info.length.length > opts.stage + 1)) {
				// Client is approx. 15 units off
				opts.distance = Math.max(0, myPosition.loc.dist2D(opts.dest) - 15)
		...
		// Handle special dcTypes
		const isSpecialDcType = ['defence', 'movingdefence', 'movingcharge', 'presshit'].includes(info.dcType)
		if(isSpecialDcType && opts.stage === ((info.length && (info.length.length || 1)) || 0)) {
			// Handle movingcharge with autoRelease
			if(info.dcType === 'movingcharge' && info.autoRelease !== undefined) {
				...
				if(info.autoRelease === 0) {
					...
					const timeout = Math.round(info.autoRelease / realSpeed)
					stageEndTimeout = mod.setTimeout(stageEnd, timeout)
				}
			}

			// Set overcharge timeout
			const overchargeTimeout = Math.round(info.overcharge / realSpeed)
			effectsTimeouts.push(mod.setTimeout(() => {
				if(clientAction) clientAction.overcharge = true
			}, overchargeTimeout))

			// Set server timeout if needed
			if(!noTimeout) {
				serverTimeout = mod.setTimeout(sendActionEnd, getServerTimeout(), 6)
			}
			return
		}
		...
		if(!noTimeout) {
			...
			if(length > serverTimeoutTime) {
				serverTimeout = mod.setTimeout(sendActionEnd, serverTimeoutTime, 6)
			}
		}

		// Handle multi-stage skills
		if(multiStage) {
			// Check for in-place distance
			if(!opts.moving) {
				const inPlaceDistance = get(info, 'inPlace', 'distance', opts.stage)
				if(inPlaceDistance !== undefined) opts.distance = inPlaceDistance
			}

			// Set up next stage if there are more stages
			if(opts.stage + 1 < info.length.length) {
				opts.stage += 1
				stageEnd = sendActionStage.bind(null, opts)
				stageEndTime = now + length
				stageEndTimeout = mod.setTimeout(stageEnd, length)
				return
			}
		}
		else if(!opts.moving) {
			// Check for in-place distance for single-stage skills
			const inPlaceDistance = get(info, 'inPlace', 'distance')
			if(inPlaceDistance !== undefined) opts.distance = inPlaceDistance
		}

		// Handle dash-type skills
		...
		// Handle special dcTypes again (for next stage)
		if(isSpecialDcType) {
			opts.stage += 1
			stageEnd = sendActionStage.bind(null, opts)
			stageEndTime = now + length
		...
		// Set up action end
		const endReason = info.dcType === 'dash' ? 39 : 0
		stageEnd = sendActionEnd.bind(null, endReason, opts.distance * opts.distanceMult, moveDir)
		stageEndTime = now + length
		stageEndTimeout = mod.setTimeout(stageEnd, length)
	}
	...
	// Optimized sendActionEnd function
	function sendActionEnd(type, distance, dir) {
		// Clear all stage-related timeouts and effects
		clearStage()

		// Early return if no client action
		...
		if(mod.settings.debug.skills) {
			const debugMsg = [
				'<* S_ACTION_END',
				skillId(clientAction.skill),
				type || 0,
				degrees(myPosition.w),
				(distance || 0) + 'u'
			].join(' ')
			debug(debugMsg)
		}
		...
		if(oopsPosition && (mod.settings.skills.forceClipStrict || !myPosition.action)) {
			sendInstantMove(oopsPosition)
		} else {
			movePlayer(distance, dir)
		}
		...
		mod.toClient('S_ACTION_END', 5, packetData)

		// Handle emulated skill
		if(clientAction.id === actionNumber) {
			const info = skillInfo(clientAction.skill)
			if(info) {
				// Handle abnormal end consumption
				if(info.consumeAbnormalEnd) {
					if(Array.isArray(info.consumeAbnormalEnd)) {
						for(const id of info.consumeAbnormalEnd) {
							abnormality().remove(id)
						}
					} else {
				...
				// Handle dash-type skills
				if(info.dcType === 'dash') {
					lastEndPosition = Object.assign({}, myPosition)
				}
			}
		} else {
			lastEndedId = clientAction.id
		}

		// Increment action number with overflow protection
		actionNumber++
		if(actionNumber > 0xffffffff) {
			actionNumber = 0x80000000
		}
	...
	function updatePosition(event, action, special) {
		// Use current action if no event provided
		event = event || clientAction
		
		// Create new position object to avoid reference issues
		myPosition = {
			loc: event.loc,
			// Handle special case for w (rotation)
			w: special ? (event.w || myPosition.w) : event.w,
			action
		}
	}

	// Optimized retry function with better timeout handling
	function retry(type, skill, cb) {
		...
		// Early return if skill has noRetry flag
		if (skill.noRetry) return

		// Cache skill and settings
		const thisSkill = clientAction.skill
		const isLongRetry = !!skill.longRetry
		const count = mod.settings.skills[isLongRetry ? 'longRetryCount' : 'retryCount']
		const ms = mod.settings.skills[isLongRetry ? 'longRetryMs' : 'retryMs']
		
		// Skip retry logic if count is 0
		if (count <= 0) return

		// Use closure for iteration counter to avoid recreating function
		let iter = 0
		const retryIter = () => {
			...
			retryTimer[type] = mod.setTimeout(() => {
				// Only retry if action is still valid
				if (clientAction && clientAction.skill.equals(thisSkill) && cb()) {
					retryIter()
				} else {
					retryTimer[type] = null
				}
	...
	// Optimized movePlayer function
	function movePlayer(distance, dir) {
		// Only move if we have a distance and not in an action
		if (distance && !myPosition.action) {
			applyDistance(myPosition, distance, dir)
		}
	}

	// Optimized applyDistance function
	function applyDistance(pos, dist, dir = 0) {
		// Skip calculation if distance is 0
		if (dist === 0) return pos
		
		// Apply rotation and add to position
	...
	function skillInfo(skill) {
		...
		if(skillsCache.size >= SKILL_CACHE_LIMIT) {
			// Clear 20% of the cache when it gets too large
			const keysToDelete = Array.from(skillsCache.keys()).slice(0, Math.floor(SKILL_CACHE_LIMIT * 0.2))
			for(const key of keysToDelete) skillsCache.delete(key)
		}

		// Calculate skill components once
		...
		// Create lookup array - ordered by least specific to most specific
		const info = [
		...
		// Note: Exact skill (or group + sub) must be specified for prediction to be enabled
		// This helps to avoid breakage for edge cases
		if(info[1] || info[10]) {
			// Merge all info objects efficiently
			cached = Object.assign({}, ...info.filter(Boolean))
	...
	// Optimized server timeout calculation
...
// Efficient property lookup with early returns
...
function debug(msg) {
	...
	console.log(cachedTimestampString, msg)
}

// Pre-calculate constants
const PI_180 = Math.PI / 180

// Optimized angle formatter
function degrees(w) {
	return Math.round(w / PI_180) + '\xb0'
}
...
// Skill ID cache for better performance
