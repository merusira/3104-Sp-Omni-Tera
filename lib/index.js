...
module.exports = function SkillPrediction(mod) {
  ...
  try {
    ...
    // Add destructor to clean up resources when module is unloaded
    mod.destructor = () => {
      try {
        // Clear dispatch map to prevent circular references
        if (subMod.clearDispatchMap) {
          subMod.clearDispatchMap(mod.dispatch)
          console.log('[skill-prediction] Cleared dispatch map during module unload')
        }
      } catch (e) {
        console.error('[skill-prediction] Error during module cleanup:', e)
      }
    }
